...
// Send normalized commands to LEFT and RIGHT motors in a single synchronized call.
// Commands are clamped to [-1,1], inversion applied per motor.
// Position targets are updated and velocity commands computed from position error.
...
// Reset position tracking: re-initialize accumulators to current encoder positions
// and clear all position targets. Use to recover from any drift or desync.
