...
void startCapture(uint32_t nSamples, bool streamCsv = true,
                  bool statsOnly = false);

// Stop an ongoing capture (if any). This will finalize stats and print summary.
void stopCapture();

// Called by SystemTasks or IMU loop to submit the latest sample (pitch_deg,
// pitch_rate_deg, left_cmd, right_cmd, ts_ms)
...
// Register a callback invoked when a capture finishes (called from
// stopCapture()). The callback runs in the context of the caller of stopCapture
// (IMU task), so it should be short and non-blocking.
