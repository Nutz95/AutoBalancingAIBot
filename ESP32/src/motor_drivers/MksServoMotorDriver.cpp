...
MksServoMotorDriver::MksServoMotorDriver()
    : m_left{LEFT_MOTOR_ID, LEFT_MOTOR_INVERT != 0, 0.0f, 0, 0, 0, 0, false},
      m_right{RIGHT_MOTOR_ID, RIGHT_MOTOR_INVERT != 0, 0.0f, 0, 0, 0, 0, false},
      m_enabled(false),
...
int32_t MksServoMotorDriver::readEncoder(MotorSide side) {
    ...
    uint8_t response[10];
    if (readResponse(state.id, 0x31, response, 10)) {
...
float MksServoMotorDriver::readSpeed(MotorSide side) {
    // Current implementation doesn't provide a direct filtered speed read over serial
    // without significant bus penalty. Returning 0.0f as default.
    return 0.0f;
}
...
void MksServoMotorDriver::scanBus() {
    ...
    for (uint8_t id = 1; id <= 15; ++id) {
        ...
        uint8_t response[10];
        bool success = readResponse(id, 0x31, response, 10);
        xSemaphoreGive(m_busMutex);
...
void MksServoMotorDriver::dumpMotorRegisters(uint8_t id) {
    ...
    for (const auto& r : regs) {
        ...
        uint8_t resp[16];
        bool success = readResponse(id, r.code, resp, r.len);
        xSemaphoreGive(m_busMutex);
...
void MksServoMotorDriver::sendSpeedCommand(uint8_t id, float normalized_speed, bool invert) {
    ...
    if (now - state.last_speed_log_ms >= 500) {
        LOG_PRINTF(abbot::log::CHANNEL_MOTOR,
                   "mks_servo: ID 0x%02X speed_uint=%u acc=%u dir=%u frame=%02X %02X %02X %02X %02X %02X %02X\n",
                   id, speed_uint, (uint8_t)MKS_SERVO_ACCEL, dir, frame[0], frame[1], frame[2], frame[3], frame[4], frame[5], frame[6]);
        state.last_speed_log_ms = now;
    }
    ...
    if (!readResponse(id, 0xF6, ack_buffer, 5)) {
        LOG_PRINTF(abbot::log::CHANNEL_MOTOR, "mks_servo: WARN: No ACK for speed command to ID 0x%02X\n", id);
    } else {
        // Log ACK status if it's not success (0x01)
        if (ack_buffer[3] != 0x01) {
            LOG_PRINTF(abbot::log::CHANNEL_MOTOR, "mks_servo: ID 0x%02X ACK status=0x%02X\n", id, ack_buffer[3]);
        }
    }
...
void MksServoMotorDriver::sendFunctionCommand(uint8_t id, uint8_t function_code, const uint8_t* data, size_t length) {
    ...
    uint8_t ack_buffer[5];
    if (readResponse(id, function_code, ack_buffer, 5)) {
...
bool MksServoMotorDriver::verifyConfig(uint8_t id, uint8_t function_code, uint8_t expected_value, const char* label) {
    ...
    if (readResponse(id, function_code, resp, expected_len)) {
...
bool MksServoMotorDriver::readResponse(uint8_t id, uint8_t function_code, uint8_t* out_data, size_t expected_length) {
    // Blocking read with timeout for telemetry
