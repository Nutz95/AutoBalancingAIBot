...
Madgwick::Madgwick()
: q0_(1.0f), q1_(0.0f), q2_(0.0f), q3_(0.0f), beta_(0.1f), last_pitch_(0.0f), last_dt_(0.0f), last_pitch_rate_(0.0f) {}

Madgwick::Madgwick(const FusionConfig &cfg)
: Madgwick() {
    begin(cfg);
}
...
void Madgwick::update(float gx, float gy, float gz, float ax, float ay, float az, float dt) {
    ...
    q0_ += dq0; q1_ += dq1; q2_ += dq2; q3_ += dq3;
...
float Madgwick::getPitchRate() const {
    return last_pitch_rate_;
}
