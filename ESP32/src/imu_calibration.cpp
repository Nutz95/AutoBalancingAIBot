...
bool saveCalibration(const Calibration &cal) {
  ...
  if (!prefs.begin("imu", false))
    return false;
  bool ok =
      prefs.putBytes("cal", &cal, sizeof(Calibration)) == sizeof(Calibration);
  prefs.end();
  return ok;
}
...
void applyCalibrationToSample(struct abbot::IMUSample &s) {
  ...
  // runtime bias is also in use.
  s.ax -= g_cal.accel_offset[0];
  s.ay -= g_cal.accel_offset[1];
  s.az -= g_cal.accel_offset[2];
}

static void computeMeanStd(const float *samples, int n, double &mean,
                           double &std) {
  double sum = 0.0;
  for (int i = 0; i < n; i++)
    sum += samples[i];
  ...
  double s = 0.0;
  for (int i = 0; i < n; i++)
    s += (samples[i] - mean) * (samples[i] - mean);
...
bool startGyroCalibration(class abbot::BMI088Driver &driver, int nSamples) {
  ...
  LOG_PRINTF(abbot::log::CHANNEL_IMU,
             "CALIB START GYRO: sampling %d samples...\n", nSamples);
  unsigned long startMs = millis();
  LOG_PRINTF(abbot::log::CHANNEL_IMU, "CALIB TIME START ms=%lu\n", startMs);

  // Use Welford's online algorithm to compute mean and variance without large
  // allocations
  double mean_x = 0.0, mean_y = 0.0, mean_z = 0.0;
  double m2_x = 0.0, m2_y = 0.0, m2_z = 0.0;
  int count = 0;
  abbot::IMUSample sample;
  // Progress reporting: print every ~1% (or at least once per sample if
  // nSamples<100)
  ...
  while (count < nSamples) {
    if (xQueueReceive(q, &sample, portMAX_DELAY) == pdTRUE) {
      ...
      double dz = sample.gz;
      count++;
      double delta = dx - mean_x;
      mean_x += delta / count;
      m2_x += delta * (dx - mean_x);
      delta = dy - mean_y;
      mean_y += delta / count;
      m2_y += delta * (dy - mean_y);
      delta = dz - mean_z;
      mean_z += delta / count;
      m2_z += delta * (dz - mean_z);
      if ((count % progressInterval) == 0) {
        int pct = (count * 100) / nSamples;
        if (pct > 100)
          pct = 100;
        LOG_PRINTF(abbot::log::CHANNEL_IMU,
                   "CALIB PROGRESS GYRO %d%% (%d/%d)\n", pct, count, nSamples);
      }
    } else {
      ...
      vQueueDelete(q);
      if (motor_was_enabled)
        abbot::motor_control::enableMotors();
      g_is_calibrating = false;
      return false;
    }
  }
  ...
  if (std_x > GYRO_STD_THRESHOLD || std_y > GYRO_STD_THRESHOLD ||
      std_z > GYRO_STD_THRESHOLD) {
    LOG_PRINTF(abbot::log::CHANNEL_IMU,
               "CALIB FAIL gyro unstable: std=(%.6f,%.6f,%.6f)\n", std_x, std_y,
               std_z);
    unsigned long endMs = millis();
    LOG_PRINTF(abbot::log::CHANNEL_IMU,
               "CALIB TIME END ms=%lu elapsed_ms=%lu\n", endMs,
               (endMs - startMs));
    detachCalibrationQueue();
    vQueueDelete(q);
    if (motor_was_enabled)
...
bool startAccelCalibration(class abbot::BMI088Driver &driver, int nSamples) {
  ...
  g_is_calibrating = true;
  LOG_PRINTF(abbot::log::CHANNEL_IMU,
             "CALIB START ACCEL: sampling %d samples...\n", nSamples);
  unsigned long startMs = millis();
  LOG_PRINTF(abbot::log::CHANNEL_IMU, "CALIB TIME START ms=%lu\n", startMs);
  ...
  while (count < nSamples) {
    if (xQueueReceive(q, &sample, portMAX_DELAY) == pdTRUE) {
      ...
      double az = sample.az;
      count++;
      double delta = ax - mean_x;
      mean_x += delta / count;
      m2_x += delta * (ax - mean_x);
      delta = ay - mean_y;
      mean_y += delta / count;
      m2_y += delta * (ay - mean_y);
      delta = az - mean_z;
      mean_z += delta / count;
      m2_z += delta * (az - mean_z);
      // Progress reporting for accel
      int progressIntervalA = nSamples / 100;
      if (progressIntervalA < 1)
        progressIntervalA = 1;
  ...
  if (std_x > ACCEL_STD_THRESHOLD || std_y > ACCEL_STD_THRESHOLD ||
      std_z > ACCEL_STD_THRESHOLD) {
    LOG_PRINTF(abbot::log::CHANNEL_IMU,
               "CALIB FAIL accel unstable: std=(%.6f,%.6f,%.6f)\n", std_x,
               std_y, std_z);
    unsigned long endMs = millis();
    LOG_PRINTF(abbot::log::CHANNEL_IMU,
               "CALIB TIME END ms=%lu elapsed_ms=%lu\n", endMs,
  ...
  // For a single-position calibration, expected gravity vector depends on
  // orientation. Here we assume Z-up (common case): expected g = +9.80665 m/s^2
  // on Z.
  ...
  bool saved = saveCalibration(g_cal);
  LOG_PRINTF(abbot::log::CHANNEL_IMU,
             "CALIB DONE accel_offset=%.6f,%.6f,%.6f\n", g_cal.accel_offset[0],
             g_cal.accel_offset[1], g_cal.accel_offset[2]);
  unsigned long endMs = millis();
  LOG_PRINTF(abbot::log::CHANNEL_IMU, "CALIB TIME END ms=%lu elapsed_ms=%lu\n",
             endMs, (endMs - startMs));
  detachCalibrationQueue();
...
void dumpCalibration() {
  ...
  LOG_PRINTF(abbot::log::CHANNEL_DEFAULT,
             "CALIB DUMP gyro_bias=%.6f,%.6f,%.6f\n", g_cal.gyro_bias[0],
             g_cal.gyro_bias[1], g_cal.gyro_bias[2]);
  LOG_PRINTF(abbot::log::CHANNEL_DEFAULT,
             "CALIB DUMP accel_offset=%.6f,%.6f,%.6f\n", g_cal.accel_offset[0],
             g_cal.accel_offset[1], g_cal.accel_offset[2]);
}
...
void installCalibration(const Calibration &cal) { g_cal = cal; }
