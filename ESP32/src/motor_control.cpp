...
void disableMotors() {
  if (auto drv = abbot::motor::getActiveMotorDriver()) {
    ...
    LOG_PRINTLN(abbot::log::CHANNEL_DEFAULT, "motor_control: no active motor driver to disable");
  }
}
...
void enableMotors() {
  if (auto drv = abbot::motor::getActiveMotorDriver()) {
    ...
    LOG_PRINTLN(abbot::log::CHANNEL_DEFAULT, "motor_control: no active motor driver to enable");
  }
}

bool areMotorsEnabled() {
  if (auto drv = abbot::motor::getActiveMotorDriver()) return drv->areMotorsEnabled();
  return false;
}
