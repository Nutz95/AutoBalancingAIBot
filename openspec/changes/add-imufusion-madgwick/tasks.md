...
- [ ] Prepare hardware: stable mount, known power, emergency cutoff accessible.
- [ ] Upload firmware build that includes `CHANNEL_TUNING` CSV emission.
- [ ] Open serial monitor at `921600` and send `TUNING START` to begin CSV stream.
- [ ] Capture at least 60 seconds of CSV while the robot is stationary; save to `tuning_capture.csv`.
- [ ] Verify CSV header and columns: `timestamp_ms,pitch_deg,pitch_rad,pitch_rate_deg,pitch_rate_rad,left_cmd,right_cmd`.
- [ ] Compute mean and standard deviation of `pitch_deg` over a 10s window; expect stddev < 0.5° and mean within ±2°.
- [ ] If noisy, reduce `beta` (slower response) or increase (faster response) and repeat capture.
- [ ] Record chosen `beta`, sample_rate used, and results into a short bench report file `openspec/changes/add-imufusion-madgwick/bench_report.md`.
